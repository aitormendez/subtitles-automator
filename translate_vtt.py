...
def translate_text_ollama(text, model, lang_instruction_for_model, block_index, total_blocks, retries=3, delay=5):
    """
    ...
    prompt = f"{lang_instruction_for_model}\n\n{text.strip()}"
    print("\n" + "-"*72)
    print(f"Translating block {block_index}/{total_blocks} to {lang_code.upper()}...\n")
    print("Input text being translated:")
    print(text.strip())
    print("\nTranslated text:")
...
def process_vtt_file(input_file, output_file, model, lang_name, lang_instruction_for_model, lang_code):
    """
    Reads a VTT file, translates the dialogue, and writes a new VTT file.
    """
    try:
    ...
    with open(output_file, 'w', encoding='utf-8') as f_out:
        ...
        for i, block in enumerate(blocks):
            ...
            lines = block.split('\n')
            if len(lines) < 2:
                print(f"Skipping malformed block {i + 1}", file=sys.stderr)
                continue
            timestamp = lines[0]
            # Only subtitle content (excluding timestamp)
            original_text = "\n".join(lines[1:])

            # Convert timestamp to VTT format if needed (ensure . for ms)
            # SRT: 00:00:00,000 --> 00:00:05,000
            # VTT: 00:00:00.000 --> 00:00:05.000
            vtt_timestamp = re.sub(r'(\d{2}:\d{2}:\d{2}),(\d{3})', r'\1.\2', timestamp)

            translated_text = translate_text_ollama(
                original_text, model, lang_instruction_for_model,
                block_index=i + 1, total_blocks=total_blocks
            )

            if translated_text:
                # Clean the translated text before writing
                cleaned_text = clean_translated_text(translated_text, lang_code)
                # Omit sequence_number, as it's not part of VTT format
                f_out.write(f"{vtt_timestamp}\n")
...
if __name__ == "__main__":
    if len(sys.argv) != 5:
        print("Usage: ./translate_vtt.py <input_file.vtt> <output_file.vtt> <default_ollama_model> <lang_code>")
        print(f"Example: ./translate_vtt.py tobias.vtt tobias.fr.vtt llama3 fr")
        print("Supported language codes:", ", ".join(LANGUAGE_DETAILS.keys()))
        sys.exit(1)

    input_vtt = sys.argv[1]
    provided_output = sys.argv[2]
    output_vtt = provided_output if os.path.isabs(provided_output) else os.path.join(os.path.dirname(input_vtt), provided_output)
    ...
    process_vtt_file(input_vtt, output_vtt, ollama_model_to_use, lang_name, lang_instruction_for_model, lang_code)
