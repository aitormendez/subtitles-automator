...
def translate_text_google(text, target_lang):
    """
    Translates text using deep_translator's GoogleTranslator.
    """
    try:
        ...
        while i < total_blocks:
            ...
            for block in batch:
                ...
                if len(lines) >= 2:
                    ...
                    batch_meta.append(timestamp)
                else:
                    # Skip empty or malformed block
                    print(f"Warning: Skipping malformed block at index {i + len(batch_texts)}", file=sys.stderr)
            ...
            if translated_text:
                ...
                for idx, timestamp in enumerate(batch_meta):
...
if __name__ == "__main__":
    ...
    output_srt = os.path.join(os.path.dirname(input_srt), output_filename)
